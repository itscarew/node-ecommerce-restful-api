const User = require("../models/user.models");
const bcrypt = require("bcryptjs");
const jwt = require("jsonwebtoken");

exports.users_get_all_users = (req, res) => {
  User.find()
    .exec()
    .then((user) => {
      res.status(200).json({
        users: user,
      });
    })
    .catch((err) => {
      res.status(400).json({
        err: err.message,
      });
    });
};

exports.users_register_user = (req, res) => {
  User.find({ email: req.body.email })
    .exec()
    .then((user) => {
      if (user.length >= 1) {
        return res.status(409).json({
          err: "Email Already Exists",
        });
      } else {
        bcrypt.hash(req.body.password, 10, (err, hash) => {
          if (err) {
            return res.status(500).json({
              err: err.message,
            });
          } else {
            const user = new User({
              name: req.body.name,
              email: req.body.email,
              password: hash,
              role: req.body.role,
            });
            return user
              .save()
              .then((user) => {
                res.status(201).json({
                  message: "User Created Successfully",
                  user,
                });
              })
              .catch((err) => {
                res.status(500).json({
                  err: err.message,
                });
              });
          }
        });
      }
    });
};

exports.users_login_user = (req, res, next) => {
  User.findOne({ email: req.body.email })
    .exec()
    .then((user) => {
      if (!user) {
        return res.status(404).json({
          err: "User does not exist!",
        });
      }
      bcrypt.compare(req.body.password, user.password, (err, result) => {
        if (err) {
          return res.status(401).json({
            err: "Authentication Failed, Password Incorect",
          });
        } else if (result) {
          const token = jwt.sign(
            {
              name: user.name,
              email: user.email,
              userId: user._id,
              role: user.role,
            },
            "secret"
            // { expiresIn: "1h" }
          );
          return res.status(200).json({
            message: "Authentication Successful, Logged In",
            token: token,
            user,
          });
        } else
          res.status(401).json({
            err: "Authentication Failed, Can't Log in",
          });
      });
    })
    .catch((err) => {
      res.status(400).json({
        err: err.message,
      });
    });
};

exports.users_get_user_profile = (req, res) => {
  User.findOne({ _id: req.user.userId })
    .exec()
    .then((user) => {
      if (!user) {
        res.status(404).json({ err: "No user is Logged In" });
      } else res.status(200).json({ user });
    })
    .catch((err) => {
      res.status(400).json({ err: err.message });
    });
};

exports.users_delete_user_profile = (req, res) => {
  User.deleteOne({ _id: req.user.userId })
    .exec()
    .then(() => {
      res.status(200).json({
        message: "You just deleted your account.",
      });
    })
    .catch((err) => {
      res.status(404).json({
        err: err.message,
      });
    });
};

exports.users_update_user_profile = (req, res) => {
  const { userId } = req.user;
  const updateOps = {};
  for (const ops of req.body) {
    updateOps[ops.propName] = ops.value;
  }
  User.updateOne({ _id: userId }, { $set: updateOps })
    .exec()
    .then((user) => {
      res.status(200).json({
        message: "User details updated succesfully",
        user,
      });
    })
    .catch((err) => {
      res.status(400).json({
        err: err.message,
      });
    });
};

exports.users_update_user_profile_password = (req, res) => {
  const { userId } = req.user;
  const { password } = req.body;
  bcrypt.hash(password, 10, (err, hash) => {
    if (err) {
      return res.status(404).json({
        err: err.message,
      });
    } else {
      User.updateOne({ _id: userId }, { $set: { password: hash } })
        .exec()
        .then((user) => {
          res.status(200).json({
            message: "User password updated succesfully",
            user,
          });
        })
        .catch((err) => {
          res.status(400).json({
            err: err.message,
          });
        });
    }
  });
};

exports.users_get_any_user = (req, res) => {
  const { userId } = req.params;

  User.findOne({ _id: userId })
    .exec()
    .then((user) => {
      if (!user) {
        res.status(404).json({
          err: "User does not exist !!",
        });
      } else {
        res.status(200).json({
          message: "User found",
          user,
        });
      }
    })
    .catch((err) => {
      res.status(400).json({
        err: err.message,
      });
    });
};

exports.users_delete_any_user = (req, res) => {
  const { userId } = req.params;
  User.deleteOne({ _id: userId })
    .exec()
    .then(() => {
      res.status(200).json({
        message: "User Account Deleted",
      });
    })
    .catch((err) => {
      res.status(404).json({
        err: err.message,
      });
    });
};
